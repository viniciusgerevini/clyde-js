...
export const LogicInterpreter = (mem: MemoryManager): LogicInterpreterInstance => {
  const expressionHandlers: Handler = {
    equal: (cond: ExpressionNode) =>
      getNodeValue(cond.elements[0]) === getNodeValue(cond.elements[1]),
    not_equal: (cond: ExpressionNode) =>
      getNodeValue(cond.elements[0]) !== getNodeValue(cond.elements[1]),
    greater_than: (cond: ExpressionNode) =>
      getNodeValue(cond.elements[0]) > getNodeValue(cond.elements[1]),
    greater_or_equal: (cond: ExpressionNode) =>
      getNodeValue(cond.elements[0]) >= getNodeValue(cond.elements[1]),
    less_than: (cond: ExpressionNode) =>
      getNodeValue(cond.elements[0]) < getNodeValue(cond.elements[1]),
    less_or_equal: (cond: ExpressionNode) =>
      getNodeValue(cond.elements[0]) <= getNodeValue(cond.elements[1]),
    and: (cond: ExpressionNode) =>
      checkCondition(cond.elements[0]) && checkCondition(cond.elements[1]),
    or: (cond: ExpressionNode) =>
      checkCondition(cond.elements[0]) || checkCondition(cond.elements[1]),
    ...
    error: (cond: ExpressionNode) => {
      throw new Error(`Unknown expression "${cond.name}"`);
    },
  };
  ...
  const operationHandlers: Handler = {
    ...
    assign_init: (name: string, value: any) =>
      mem.getVariable(name) == undefined ? mem.setVariable(name, value) : mem.getVariable(name),
    assign_sum: (name: string, value: number) =>
      mem.setVariable(name, mem.getVariable<number>(name, 0) + value),
    assign_sub: (name: string, value: number) =>
      mem.setVariable(name, mem.getVariable<number>(name, 0) - value),
    assign_mult: (name: string, value: number) =>
      mem.setVariable(name, mem.getVariable<number>(name, 0) * value),
    assign_div: (name: string, value: number) =>
      mem.setVariable(name, mem.getVariable<number>(name, 0) / value),
    assign_pow: (name: string, value: number) =>
      mem.setVariable(name, mem.getVariable<number>(name, 0) ** value),
    assign_mod: (name: string, value: number) =>
      mem.setVariable(name, mem.getVariable<number>(name, 0) % value),
    error: (_n: any, _v: any, a: AssignmentNode) => {
  ...
  const handleAssignement = (assignment: AssignmentNode): any => {
    ...
    return (operationHandlers[assignment.operation] || operationHandlers["error"])(
      variable.name,
      value,
      assignment,
    );
