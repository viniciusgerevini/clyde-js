...
type WorkingDoc = Omit<RuntimeClydeDocumentRoot, "type"> & {
  type: "document" | "linked_document";
} & WorkingNode &
  DocExtraWorkingFields;
...
export function Interpreter(
  ...
  const nodeHandlers: { [type: string]: Function } = {
    ...
    conditional_content: (node: ConditionalContentNode, fallback: any) =>
      handleConditionalContent(node, fallback),
  ...
  const shuffleWithMemory = (variations: VariationsNode & WorkingNode, mode: string): number => {
    ...
    const remainingOptions: (ContentNode & WorkingNode)[] = variations.content.filter(
      (a: ContentNode & WorkingNode) => !visitedItems.includes(a._index!),
    );
  ...
  const handleNextNode = (node: any, fallback?: any) =>
    (nodeHandlers[node.type] || nodeHandlers["error"])(node, fallback);
  ...
  const handleDivert = (divert: DivertNode): ContentReturnType => {
    ...
    if (divert.target === "<parent>") {
      while (
        !["document", "block", "option", "options", "linked_document"].includes(
          stackHead().current.type,
        )
      ) {
        stack.pop();
      }
  ...
  const handleOptionsNode = (optionsNode: OptionsNode & WorkingNode): DialogueOptions => {
    ...
    return {
      ...
      text: replaceVariables(
        translateText(optionsNode.name as string, optionsNode.id, optionsNode.id_suffixes),
      ),
      options: options
        .map((t: ActionContentNode | OptionNode) => (t.type === "action_content" ? t.content : t))
        .map((t: OptionNode & WorkingNode) => ({
  ...
  const handleVariations = (
    variations: VariationsNode & WorkingNode,
    attempt = 0,
  ): ContentReturnType => {
    ...
    if (
      variations.content[next].content.length === 1 &&
      variations.content[next].content[0].type === "conditional_content"
    ) {
  ...
  const triggerEvents = (events: EventsNode) => {
    events.events.forEach((event: EventNode) => {
      listeners.triggerEvent(EventType.EVENT_TRIGGERED, {
        name: event.name,
  ...
  const handleConditionalContent = (
    conditionalNode: ConditionalContentNode,
    fallbackNode = stackHead().current,
  ) => {
  ...
  const selectOption = (contentIndex: number): void => {
    ...
    if (node.current.type === "options") {
      ...
      (content[contentIndex].content as ContentNode & WorkingNode)._index =
        content[contentIndex]._index;
  ...
  const getVisibleOptions = (
    options: OptionsNode,
  ): ((OptionNode | WorkingActionContentNode) & WorkingNode)[] => {
  ...
  const prepareOption = (
    option: (WorkingActionContentNode | ConditionalContentNode | OptionNode) & WorkingNode,
    index: number,
  ): any => {
    if (!option._index) {
