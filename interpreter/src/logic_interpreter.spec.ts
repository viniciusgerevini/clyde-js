...
describe("Logic Interpreter", () => {
  ...
  it("throws when unknown condition", () => {
    ...
    expect(() => logic.checkCondition(wrongCondition)).toThrow(
  ...
  it("throws when unknown expression", () => {
    ...
    expect(() => logic.checkCondition(wrongCondition)).toThrow(
      /Unknown expression "unknownExpression"/,
    );
  });

  it("throws when unknown expression", () => {
    const wrongAssignment: any = {
      operation: "unknownOperation",
      variable: {},
      value: { type: "literal" },
    };
    expect(() => logic.handleAssignement(wrongAssignment)).toThrow(
