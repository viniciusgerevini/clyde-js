...
export function csvConverter(
  clydeDocument: ClydeDocumentRoot,
  ...
  csvLines = csvLines.concat(
    clydeDocument.blocks.reduce((acc, next) => {
...
function parseOptionNode(option: OptionNode, cfg: NodeOptions): string[] {
  ...
  if (
    !option.content.content?.length ||
    option.content.content[0].id !== option.id ||
    (!option.id && option.name !== option.content.content[0].value)
  ) {
