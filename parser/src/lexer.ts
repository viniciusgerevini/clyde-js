...
export function tokenize(input: string): TokenList {
  ...
  const handleLink = () => {
    ...
    return {
      ...
      line,
      column: initialColumn,
      value: JSON.stringify({ name: linkName, path: linkPath }),
    };
  };
  ...
  const handleStartVariations = (): Token[] => {
    ...
    const tokens: Token[] = [{ token: TOKENS.BRACKET_OPEN, line, column: initialColumn }];
  ...
  const handleLogicDescriptiveOperator = (
    value: string,
  ...
  function getNextToken(): TokenHandlerReturn {
    if (
      !isCurrentMode(LexerMode.QSTRING) &&
      input[position] === "-" &&
      input[position + 1] === "-"
    ) {
    ...
    if (
      !isCurrentMode(LexerMode.LOGIC) &&
      ((column === 0 && input[position].match(/[\t ]/)) || (column === 0 && indent.length > 1))
    ) {
      return handleIndent();
    }
